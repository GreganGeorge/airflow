...
def write_to_minio(table_name, records):
    if not records:
        return
    df = pd.DataFrame(records) # converts the list of JSON values to rows and columns
    date_str = datetime.now().strftime('%Y-%m-%d')
    file_path = f'{table_name}_{date_str}.parquet'
    df.to_parquet(file_path, engine='fastparquet', index=False) # This converts the table into the Parquet format and stores in the local disk
...
for message in consumer: # checks infinitely for messages from Kafka
    topic = message.topic 
    event = message.value
